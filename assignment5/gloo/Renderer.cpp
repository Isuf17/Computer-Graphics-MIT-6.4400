...
void Renderer::RenderShadow(const RenderingInfo& rendering_info, const LightComponent *light_component) const {
  BindGuard shader_bg(shadow_shader_.get());
  BindGuard framebuffer_bg(framebuffer_.get());
  ...
  for (const auto& pr : rendering_info) {
      ...
      shadow_shader_->SetMatrix(world_to_light_NDC_matrix);
      robj_ptr->Render();
  }
}
...
void Renderer::RenderScene(const Scene& scene) const {
...

  {
    ...
    // https://www.khronos.org/opengl/wiki/Early_Fragmendret_Test#Optimization
  ...
  for (size_t light_id = 0; light_id < light_ptrs.size(); light_id++) {
    ...
    if (light_ptrs[light_id]->CanCastShadow()) {
      RenderShadow(rendering_info, light_ptrs[light_id]);
    }
    ...
    for (const auto& pr : rendering_info) {
      ...
      glm::mat4 view_matrix = light_ptrs[light_id]->GetNodePtr()->GetTransform().GetLocalToWorldMatrix();
      glm::mat4 world_to_light_NDC_matrix = kLightProjection*glm::inverse(view_matrix);
