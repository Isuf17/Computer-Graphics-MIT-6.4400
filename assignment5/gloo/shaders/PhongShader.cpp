...
void PhongShader::SetTargetNode(const SceneNode& node,
  ...
  const auto& ambient_tex = material_ptr->GetAmbientTexture();
  if (ambient_tex != nullptr) {
      (*ambient_tex).BindToUnit(0);
      SetUniform("ambient_tex", 0);
  }
  const auto& diffuse_tex = material_ptr->GetDiffuseTexture();
  if (diffuse_tex != nullptr) {
      (*diffuse_tex).BindToUnit(1);
      SetUniform("diffuse_tex", 1);
  }
  const auto& specular_tex = material_ptr->GetSpecularTexture();
  if (specular_tex != nullptr) {
      (*specular_tex).BindToUnit(2);
      SetUniform("specular_tex", 2);
  }
}
...
void PhongShader::SetShadowMapping(
  ...
  shadow_texture.BindToUnit(2);
  SetUniform("shadow_map", 2);
}
}  // namespace GLOO
