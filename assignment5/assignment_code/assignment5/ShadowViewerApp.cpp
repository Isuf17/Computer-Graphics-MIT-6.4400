...
void ShadowViewerApp::SetupScene() {
  ...
  for (int i = 0; i < mesh_data.groups.size(); i++) {
    MeshGroup mesh_group = mesh_data.groups[i];    

    auto mesh_node = make_unique<SceneNode>();

    mesh_node->CreateComponent<ShadingComponent>(shader);
    mesh_node->CreateComponent<MaterialComponent>(mesh_group.material);

    auto& rc = mesh_node->CreateComponent<RenderingComponent>(vertex_obj);
