...
glm::mat4 Transform::GetLocalToAncestorMatrix(SceneNode* ancestor) const {
  ...
  return node_.GetParentPtr()->GetTransform().GetLocalToAncestorMatrix(ancestor) * local_transform_mat_; /* recursively find ancestor */
}

glm::mat4 Transform::GetLocalToWorldMatrix() const {
  return GetLocalToAncestorMatrix(nullptr); /* nullptr is the ptr for the root */
}
